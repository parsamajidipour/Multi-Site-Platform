...
function endpointForPath(pathname, localeOverride) {
  ...
  if (path === "/") return `/api/public/sites/${SITE_KEY}/homepage/?locale=${locale}`;
  const slug = path.replace(/^\/+/, "").split("/").map(encodeURIComponent).join("/");
...
async function fetchJsonWithTimeout(url, timeoutMs = DEFAULT_TIMEOUT_MS, options = {}) {
  ...
  try {
    ...
    if (!data || typeof data !== "object") throw new Error("CMS returned malformed JSON");
    return data;
  } finally {
...
export async function fetchCmsPage(pathname, locale) {
  ...
  if (!baseUrl) throw new Error("VITE_CMS_API_URL is not configured");
  return fetchJsonWithTimeout(`${baseUrl}${endpointForPath(pathname, locale)}`);
}

export async function fetchCmsBlogPosts(locale) {
  const baseUrl = normalizeBaseUrl(import.meta.env.VITE_CMS_API_URL);
  if (!baseUrl) throw new Error("VITE_CMS_API_URL is not configured");
  const localeParam = encodeURIComponent(currentLocale(locale));
  return fetchJsonWithTimeout(`${baseUrl}/api/public/sites/${SITE_KEY}/blog/?locale=${localeParam}`);
}
